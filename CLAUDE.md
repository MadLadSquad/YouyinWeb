...
- **Storage:** All deck/session state lives in `localStorage` under two keys:
  ...
  - `youyinGameModifiers` — `{ extensive, levelReduce }`. `index.js#main()` defensively initializes both keys and reloads the page if either is missing — be aware that `saveToLocalStorage` / `saveGameModifiers` mutate-then-save the global, and a reload is normal control flow here.
