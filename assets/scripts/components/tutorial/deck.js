...
async function tutRunDeckTour()
{
    ...
    if (tutIsReplay())
    {
        ...
        steps[steps.length - 1].onNext = () => { tutSetStep(TUT.session); tutNavigate(window.pageUrl("index")); return false; };
        tutRunTour(steps);
        return;
    }
...
async function tutShowDeckReviewMembership()
{
    ...
    steps.push({
        ...
        onNext: () => { tutSetStep(TUT.session); tutNavigate(window.pageUrl("index")); return false; },
    });
